    ...
    protected static function getNextToken($string, $previous = null)
    {
        ...
        if ($string[0] === '@' && isset($string[1])) {
            ...
            // Non-quoted variable name
                ...
                preg_match('/^(@[a-zA-ZпїЅ-пїЅпїЅ-пїЅ0-9\._\$]+)/',$string,$matches);
                if ($matches) {
    ...
    protected static function highlightToken($token)
    {
        ...
        if (self::is_cli()) {
            ...
            if (defined('ENT_IGNORE')) {
                $token = htmlentities($token[self::TOKEN_VALUE],ENT_COMPAT | ENT_IGNORE ,'windows-1251');
            } else {
                $token = htmlentities($token[self::TOKEN_VALUE],ENT_COMPAT,'windows-1251');
            }
        }
